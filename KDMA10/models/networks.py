...
class ActorNetwork(torch.nn.Module):
    def __init__(self, init_std:float=.5):
        ...
        self.model = Policy(agent_dim=4, neighbor_dim=5, out_dim=2)
    ...
    def forward(self, s, neighbors=None, n_neighbors=None):
        if neighbors is None:
            if s.ndim == 1:
                agent, neighbors = s[:4], s[4:].view(-1, 5)
            else: #s.ndim == 2
                agent, neighbors = s[:, :4], s[:, 4:].view(s.size(0), -1, 5)
...
class CriticNetwork(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.n_neighbors = None # sequence length for batch update
        self.model = Policy(agent_dim=4, neighbor_dim=5, out_dim=1)
